...
using wowee::ui::glueBackdropFraming;
...
TEST_CASE("a camera looking along -X is a half turn of yaw and no pitch") {
    ...
    const auto f = glueBackdropFraming(eye, at, 1.4f, 16.0f / 9.0f);
    REQUIRE(f.usable);
...
TEST_CASE("pitch is the rise of the look direction, not its angle in the plane") {
    ...
    const auto f = glueBackdropFraming(eye, at, 1.4f, 1.0f);
    REQUIRE(f.usable);
    ...
    const auto g = glueBackdropFraming(eye, below, 1.4f, 1.0f);
    REQUIRE(g.usable);
...
TEST_CASE("the field of view is converted from diagonal to vertical") {
    ...
    const auto square = glueBackdropFraming(eye, at, 1.4f, 1.0f);
    ...
    const auto wide = glueBackdropFraming(eye, at, 1.4f, 16.0f / 9.0f);
    REQUIRE(wide.usable);
...
TEST_CASE("a camera that cannot frame anything says so") {
    ...
    SECTION("looking at the point it stands on") {
        ...
        CHECK_FALSE(glueBackdropFraming(eye, at, 1.4f, 1.0f).usable);
    }
    SECTION("no field of view at all") {
        const float at[3] = {2.0f, 2.0f, 3.0f};
        CHECK_FALSE(glueBackdropFraming(eye, at, 0.0f, 1.0f).usable);
    }
    SECTION("a field of view larger than a camera can have") {
        const float at[3] = {2.0f, 2.0f, 3.0f};
        CHECK_FALSE(glueBackdropFraming(eye, at, 6.0f, 1.0f).usable);
    }
    SECTION("a view with no width") {
        const float at[3] = {2.0f, 2.0f, 3.0f};
        CHECK_FALSE(glueBackdropFraming(eye, at, 1.4f, 0.0f).usable);
    }
