...
- **Not shipped:** S4 sun shafts, and M3a normal maps for doodads and terrain beyond the tangent routine - the `NormalMapCache`, the tangent attribute on the M2 and terrain vertices, and the shader paths that would read them. Geomorph was dropped as the phase file's cut order says to. See `docs/evidence/phase-01/README.md`
