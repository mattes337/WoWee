...
- The shadow casters' per-texture descriptor sets are freed once a frame rather than once a cascade. `M2Renderer::renderShadow` and `CharacterRenderer::renderShadow` reset this frame slot's shadow-texture descriptor pool on entry, and the cascaded pass calls each of them once per cascade into one command buffer - so from the second cascade on, that reset freed the sets the first cascade had already bound, and every command recorded afterwards went into a command buffer the validation layer had marked invalid. It looked like a hundred and sixty errors on the one start-up frame the MSAA rebuild resets the frame synchronisation, because the cascade count is applied at start-up and because the layer stops reporting each VUID after ten; the rebuild had nothing to do with it and it was every frame at more than one cascade. `Renderer::renderShadowPass` resets both pools once, before the cascade loop, through `beginShadowFrame()`
- **Verified on a GPU, from the game's own archives.** The client reads an installation's MPQ files directly when its data directory holds no `manifest.json`, so nothing has to be extracted; `capture_scene --install <game folder>` drives that path, and every phase-01 technique has now been rendered, compared and measured through it. Five defects that only a loaded world could show were found and fixed - the deferred normal-map descriptor write was landing while command buffers still named the set, the cascaded shadow pass was resetting its caster texture pool once per cascade into a command buffer the earlier cascades had already bound sets from, PCSS was rendering as Poisson because its penumbra was computed in normalized depth, the capture tool was putting the player model on the lens, and two renders of one camera were not the same frame. `docs/evidence/phase-01/README.md` has the numbers, the noise floor each is measured against, and the two rows that are still not what the phase file asked for: terrain LOD at Tanaris changes 4.4 % of pixels against a 0.5 % threshold, and the height model's horizon at Westfall is ΔE 9 from the linear one against a rule of 3

In progress / known gaps:

- The phase-01 evidence PNGs under `docs/evidence/phase-01/img/` no longer reproduce on the machine they were made on. `e8d4af002` rebuilt from its own tree and run with the command that made them differs from `goldshire-lake-cascades.after.png` by 8.41 / 255 over 93.5 % of pixels - a flat colour shift, red +8.8, green +5.6, blue -5.8, across the whole frame rather than anything local - so whatever moved is outside this repository. Two renders of the same command by the same build still agree to 0.008 / 255 over 0.44 % of pixels, so the renderer itself is as reproducible as it was; it is the images that are stale, and a pixel comparison against them says nothing until they are re-rendered
- Two minutes of the client itself with validation on, on the glue screen, raises ten `vkCmdSetCheckpointNV` and ten `vkCmdWriteTimestamp` "cannot be called on this primary command buffer because vkCmdBeginRenderPass was called with VK_SUBPASS_CONTENTS_SECONDARY_COMMAND_BUFFERS" before the layer's duplicate limit silences them. It is `Renderer::endFrame` marking `post-process` on the primary while the scene pass - opened for secondaries when `parallelRecordingEnabled_` - is still open; the mark belongs after the pass is closed. Not seen through `capture_scene`, which is why five passes of phase-01 evidence never showed it
