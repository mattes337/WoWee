# Phase 20 — NVIDIA: licence review, Streamline, DLSS Super Resolution, Reflex

**One session, one commit.** Depends on: 16. Player sees: on an RTX card with the NVIDIA
...
- **Licence review, first hour, written down** in `docs/vendor-licences.md`: Streamline
  source terms vs the DLSS/NGX runtime and DLSS-G redistribution terms. Outcome A: runtime
...
```cpp
// RESERVED(phase-21, V-xess-antilag): LatencyMode's "vendor" slot table has a row for
...
- With the runtime: compare mode `--sequence` on `goldshire-inn-morning`,
  ...
  SSIM vs native recorded. Without the runtime: bit-identical to phase 19 and the row is
  greyed.
