...
struct GraphicsPresetValues {
    ...
    /// 0 = the linear start/end ramp, 1 = exponential height fog.
    ///
    /// Every preset says 1, including Low: the height model costs a handful of
    /// instructions in a shader that already runs a dozen texture fetches, and
    /// it is the one thing in this table that changes what the world *looks*
    /// like rather than how much of it is drawn. A column all the same value
    ...
    int   terrainLod;
    /// 0 = buildings and characters, which is every normal map the client ever
    /// had; 1 = doodads and the ground as well. Low says 0: the maps are
    /// generated in the background and held in video memory, and Low is the
    /// preset for a machine with neither to spare.
    int   normalMapScope;
    /// Screen-space sun shafts. Off on Low for the same reason as the column
    /// above: three half-resolution passes is little, and Low is where little
    /// still counts.
    bool  sunShafts;
};
