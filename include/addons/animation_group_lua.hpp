...
inline constexpr const char kAnimationGroupLua[] =
        ...
        "            if a.kind == 'Alpha' and self.baseAlpha then self.parent:SetAlpha(self.baseAlpha) end\n"
        "            if a.kind == 'Translation' then __WoweeSetAnimOffset(self.parent, 0, 0) end\n"
        "        end\n"
        "    end\n"
        ...
        "    if type(elapsed) ~= 'number' or elapsed ~= elapsed or\n"
        "       elapsed < 0 or elapsed >= 1 / 0 then elapsed = 0 end\n"
        "    for g in pairs(playing) do\n"
        "        if g.paused then\n"
        "        else\n"
        "            local remaining = (g.pendingElapsed or 0) + elapsed\n"
        "            g.pendingElapsed = 0\n"
        "            local boundaries = 0\n"
        "            local keepTicking = true\n"
        "            while keepTicking do\n"
        "            keepTicking = false\n"
        "            local runId = g.runId\n"
        "            local starts, duration = schedule(g)\n"
        // Stop at each loop boundary so its animation and OnLoop callbacks
        // observe that boundary. Any overshoot is consumed by later rounds.
        "            local step = remaining\n"
        "            local toBoundary = duration - (g.elapsed or 0)\n"
        "            if duration > 0 and toBoundary >= 0 and step > toBoundary then\n"
        "                step = toBoundary\n"
        "            end\n"
        "            g.elapsed = (g.elapsed or 0) + step\n"
        "            remaining = remaining - step\n"
        "            local anyRunning = g.elapsed < duration\n"
        "            local dx, dy = 0, 0\n"
        "            local alpha = g.baseAlpha or 1\n"
        "            local changesAlpha, changesOffset = false, false\n"
        "            for _, a in ipairs(g.animations) do\n"
        ...
        "                if changesAlpha then g.parent:SetAlpha(alpha) end\n"
        "                if changesOffset then __WoweeSetAnimOffset(g.parent, dx, dy) end\n"
        "            end\n"
        ...
        "                    local sameRun = g.isPlaying and g.runId == runId\n"
        "                    if remaining > 0 and duration > 0 and sameRun then\n"
        // Bound callback work in one frame without losing time. A paused run
        // keeps the same debt, while Play's new runId intentionally drops it.
        "                        if boundaries < 64 and not g.paused then\n"
        "                            keepTicking = true\n"
        "                        else\n"
        "                            g.pendingElapsed = remaining\n"
        "                        end\n"
        "                    end\n"
        "                else\n"
        "                    g:Finish()\n"
